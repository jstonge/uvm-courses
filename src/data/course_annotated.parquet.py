...
#         ror, cat_type, year_1, year_2, page_number = fname.stem.split("_")[:5]
        
#         try:
#             with open(fname) as f:
#                 out = json.loads(repair_json(f.read()))
#                 for e in out:
#                     tmp = [e['Department'], e['Number'], e['Title'], e['Description'], year]
#                     data.append(tmp)
#         except:
#             print(f"Error reading {fname}")


# df = pd.DataFrame(data, columns=['Department', 'Number', 'Title', 'Description', 'year'])
